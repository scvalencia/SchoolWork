...
La estructura básica de un documento HTML, comienza con la declaración del tipo de documento para evitar confusión entre otros formatos parecidos `<!DOCTYPE html>`, todo el documento, está contenido dentro de el elmento `html`, éste elemento, está dividido en dos secciones `head`, `body`, la primera sección, contiene información descriptiva acerca del documento mismo, es decir el `title`, *css*, `scripts`, y otro tipo de información acerca el documento mismo. `meta` define información sobre el documento mismo, en general, especifica la codificación de texto utilizada en el documento. Finalmente, `body` contiene la información que se desea el navegador despliegue.
...
Puede verse que aunque el documento *html*, mostrado para definir la página del profesor Soto no contiene la estructura minimal de un documento *html*, éste es desplegado por el navegador, esto se debe a la falta de una estructura rígida sintáctica sobre *html*. El documento *HTML* con estilo minimal es:
...
Sin embargo, la apariencia es igual.
